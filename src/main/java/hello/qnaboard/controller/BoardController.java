...
//        request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getRequestURI();
    ...
    public String board(@PathVariable BoardType boardType, @RequestParam(required = false) Integer category, @ModelAttribute BoardSearchCond boardSearchCond,
                        @RequestParam(defaultValue = "1") int page, @AuthenticationPrincipal SessionMember loginMember, Model model) {
//        model.addAttribute("boardType", boardType); (ThymeleafView는 스프링 @PathVariable uri 변수를 자동으로 모델에 담음)
    ...
    public String boardDetail(@PathVariable BoardType boardType, @PathVariable Long boardId,
    ...
    public String boardWriteForm(@PathVariable BoardType boardType, @AuthenticationPrincipal SessionMember loginMember, Model model) {
    ...
    public String writeBoard(@PathVariable BoardType boardType, @ModelAttribute @Valid BoardWriteForm boardWriteForm, BindingResult bindingResult,
                             @AuthenticationPrincipal SessionMember loginMember, Model model) {
    ...
    public String boardEditForm(@PathVariable BoardType boardType, @PathVariable Long boardId,
                                @AuthenticationPrincipal SessionMember loginMember, Model model) {
    ...
    public String editBoard(@PathVariable BoardType boardType, @PathVariable Long boardId,
                            @ModelAttribute("boardUpdateForm") @Valid BoardUpdateForm updateParam, BindingResult bindingResult,
                            @AuthenticationPrincipal SessionMember loginMember, Model model) {
        if (bindingResult.hasErrors()) {
            model.addAttribute("loginMember", loginMember);
            return "board/boardEditForm";
